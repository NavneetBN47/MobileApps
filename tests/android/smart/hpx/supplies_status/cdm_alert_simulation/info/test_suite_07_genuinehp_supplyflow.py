...
class Test_Suite_07_GenuineHP_Supplyflow(object):
    ...
    def test_01_genuinehp_supplyflow_66101(self):
        alert_title, alert_icon, alert_body = self.fc.read_supplies_status_json_files(ioref=66101, type="info")
    ...
    @pytest.mark.jupiter
    def test_02_safe_mode_recovering_66516(self):
        alert_title, alert_icon, alert_body = self.fc.read_supplies_status_json_files(ioref=66516, type="info")
        success = InformationSimulatorCDM.safe_mode_recovery_in_progress_66516(self.error_manager, color="k")
        assert success, "Failed to set safe mode recovery info"
        assert self.supplies_status.verify_printer_card_present()
        # Printer Details Page
        assert self.supplies_status.verify_info_alert_title_in_pdp_screen(alert_title)
        assert self.supplies_status.verify_info_alert_icon_in_pdp_screen(alert_icon)
