...
from MobileApps.libs.flows.android.google_docs.google_docs import GoogleDocs


pytest.app_info = "GOOGLE_DOCS"
class Test_Suite_HPPS_google_docs(object):

    @pytest.fixture(scope="class", autouse="true")
    def class_setup(self, hpps_setup ,load_printers_session):
        self = self.__class__
        self.driver, self.fc = hpps_setup
        self.google_docs = GoogleDocs(self.driver)
    ...
    @pytest.mark.parametrize('file', [const.GOOGLE_DOCS.DOCX_1])
    def test_01_google_docs_system_ui(self, file):
        """
                Description:
                ...
                Expected Result:
                     Step 3:   Verify Google Docs home screen.
        ...
        self.google_docs.open_google_docs()
        self.fc.google_signin_prompt()
        self.google_docs.verify_google_docs_home_screen()
        self.google_docs.select_search()
        self.google_docs.verify_search_screen()
        self.google_docs.search_for_file_by_name(file)
        self.google_docs.check_search_result_and_redo_the_search_if_needed(file_search=file)
        self.google_docs.select_3dot_menu()
        self.google_docs.verify_3dot_menu_screen()
        self.google_docs.select_print()
