...
@pytest.fixture(scope="session", params=["pc_device_display_control_screenshot"])
...
class Test_Suite_Localization(object):
    ...
    def test_03_pcdevice_module_display_control_C36836298(self, language):
        ...
        time.sleep(3)
        ma_misc.create_localization_screenshot_folder("pc_device_display_control_screenshot", self.attachment_path)
        self.driver.wdvr.get_screenshot_as_file(self.attachment_path + "pc_device_display_control_screenshot/{}_pc_device_pc_display_control.png".format(language))
        time.sleep(6)
        # display
        expected_display_control_text = lang_settings["actions"]["displayControl"]["featureCard"]
        actual_display_control_text = self.fc.fd["devices"].get_display_control_text()
        soft_assertion.assert_equal(actual_display_control_text, expected_display_control_text, f"Display control title text is not matching. expected string text is {expected_display_control_text}, but got {actual_display_control_text}. ")
