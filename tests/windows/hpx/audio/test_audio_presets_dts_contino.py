...
class Test_Suite_Auido_Presets_02(object):
    ...
    @pytest.mark.ota
    def test_01_audio_presets_eq_support_on_sr_solution_machine_with_internal_speaker_3_5mm_headphone_C32348167(self):
        ...
        time.sleep(2)
        assert self.fc.fd["audio"].verify_presets_text_show() is False
        assert self.fc.fd["audio"].verify_movie_checkbox_show() is False
        assert self.fc.fd["audio"].verify_voice_checkbox_show() is False
        assert self.fc.fd["audio"].verify_music_checkbox_show() is False
        time.sleep(2)
        assert self.fc.fd["audio"].verify_equalizer_text_show() is True
        assert self.fc.fd["audio"].verify_equalizer_tooltip_icon_show() is True
        assert self.fc.fd["audio"].verify_equalizer_slider_volume_show() is True
