...
#The baseline images are taken against MasadaNX with Trio Pen, so need to run against that.
@pytest.mark.usefixtures("class_setup_fixture_for_desktop")
class Test_Suite_PenControl_Images_UI_Trio(object):

    @pytest.mark.require_platform(["not available"])
    @pytest.mark.commercial
    def test_01_trio_pen_image_C53044271(self):
        # Ensure the test starts on the MyPen page; navigate there if not already.
        self.fc.check_and_navigate_to_my_pen_page()
        image_compare_result = self.fc.get_screenshot_comparison_result(self.fc.fd["pen_control"].verify_pen_lone_image, pen_type="trio")
        ...
        image_compare_result = self.fc.get_screenshot_comparison_result(self.fc.fd["pen_control"].verify_customize_buttons_pen_lwo_image_trio, barrel_type="default")
        if image_compare_result is not None:
        ...
        image_compare_result = self.fc.get_screenshot_comparison_result(self.fc.fd["pen_control"].verify_customize_buttons_pen_lwo_image_trio, barrel_type="upper")
        if image_compare_result is not None:
        ...
        image_compare_result = self.fc.get_screenshot_comparison_result(self.fc.fd["pen_control"].verify_customize_buttons_pen_lwo_image_trio, barrel_type="lower")
        if image_compare_result is not None:
        ...
        image_compare_result = self.fc.get_screenshot_comparison_result(self.fc.fd["pen_control"].verify_pen_sensitivity_lwo_image_trio)
       # Only assert if screenshot comparison actually happened
        if image_compare_result is not None:
