import time
import pytest
import logging
from MobileApps.libs.flows.windows.hpx_rebranding.flow_container import FlowContainer
from datetime import datetime, timezone
from MobileApps.libs.flows.windows.hpx_rebranding.utility.rebrand_analytics_test import RebrandAnalyticsTest

pytest.app_info = "HPX"
analytics_test = RebrandAnalyticsTest()

@pytest.mark.usefixtures("class_setup_fixture")
class Test_Suite_Smart_Experience_Analytics(object):

    @pytest.mark.analytics
    def test_01_auto_hdr_off_toggle_switch_event_C52212717(self):
        if "Maximize myHP" == self.fc.fd["devicesMFE"].verify_window_maximize():
                self.fc.fd["devicesMFE"].maximize_app()
        self.fc.fd["devicesMFE"].verify_device_card_show_up()
        self.fc.fd["devicesMFE"].click_device_card()
        self.fc.swipe_window(direction="down", distance=5)
        presence_detection_card_lone_page = self.fc.fd["devices_details_pc_mfe"].verify_presence_detection_card_lone_page()
        assert presence_detection_card_lone_page.text == "Presence detection", "Camera and presence detection card is not present."
        time.sleep(5)
        self.fc.fd["devices_details_pc_mfe"].click_presence_detection_card_lone_page()
        time.sleep(5)
        self.fc.fd["smart_experience"].verify_auto_hdr_text_ltwo_page(), "Auto HDR card text is not displayed"
        query_start_time = datetime.now(timezone.utc).isoformat()
        for i in range(5):
            current_state = self.fc.fd["smart_experience"].get_auto_hdr_toggle_button_state()
            if current_state == "1":
                self.fc.fd["smart_experience"].click_auto_hdr_toggle_button()
                logging.info(f"Clicked to turn off. Attempt {i+1}/5.")
            else:
                self.fc.fd["smart_experience"].click_auto_hdr_toggle_button()
                self.fc.fd["smart_experience"].click_auto_hdr_toggle_button()
                logging.info(f"Clicked to turn off after toggling on. Attempt {i+1}/5.")

        serial_number = self.fc.get_windows_serial_number()
        custom_filter = {    
        "version": "2.0.0",   
        "viewHierarchy": ["base:/", "mfe:/SmartExperiences/FeatureCard/"],
        "viewName": "FeatureCard", 
        "action": "OnChange",
        "actionDetail": "Off",
        "viewModule": "SmartExperiences",
        "controlName": "AutohdrToggle",
        "controlLabel": "AutohdrToggle",
        "serial_number": serial_number
        }
        analytics_test.custom_filter_json_file("resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", custom_filter)
        analytics_test.execute_analytics_test_and_validate_result(query_start_time, "resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", "vision_ai", 5)

    @pytest.mark.analytics
    def test_02_auto_hdr_on_toggle_switch_event_C52212716(self):
        self.fc.fd["smart_experience"].verify_auto_hdr_text_ltwo_page(), "Auto HDR card text is not displayed"
        query_start_time = datetime.now(timezone.utc).isoformat()
        for i in range(5):
            current_state = self.fc.fd["smart_experience"].get_auto_hdr_toggle_button_state()
            if current_state == "0":
                # If the toggle is off, click to turn it on
                self.fc.fd["smart_experience"].click_auto_hdr_toggle_button()
                logging.info(f"Clicked to turn on. Attempt {i+1}/5.")
            else:
                self.fc.fd["smart_experience"].click_auto_hdr_toggle_button()
                self.fc.fd["smart_experience"].click_auto_hdr_toggle_button()
                logging.info(f"Clicked to turn on after toggling off. Attempt {i+1}/5.")
        serial_number = self.fc.get_windows_serial_number()
        custom_filter = {    
        "version": "2.0.0",   
        "viewHierarchy": ["base:/", "mfe:/SmartExperiences/FeatureCard/"],
        "viewName": "FeatureCard", 
        "action": "OnChange",
        "actionDetail": "On",
        "viewModule": "SmartExperiences",
        "controlName": "AutohdrToggle",
        "controlLabel": "AutohdrToggle",
        "serial_number": serial_number
        }
        analytics_test.custom_filter_json_file("resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", custom_filter)
        analytics_test.execute_analytics_test_and_validate_result(query_start_time, "resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", "vision_ai", 5)

    @pytest.mark.analytics
    def test_03_attention_focus_on_toggle_switch_event_C52212718(self):
        self.fc.fd["smart_experience"].verify_attention_focus_text_ltwo_page(), "Attention Focus card text is not displayed"
        query_start_time = datetime.now(timezone.utc).isoformat()
        for i in range(5):
            current_state = self.fc.fd["smart_experience"].get_attention_focus_toggle_button_state()
            if current_state == "0":
                self.fc.fd["smart_experience"].click_attention_focus_toggle_button_toggle_button_state()
                logging.info(f"Clicked to turn on. Attempt {i+1}/5.")
            else:
                self.fc.fd["smart_experience"].click_attention_focus_toggle_button_toggle_button_state()
                self.fc.fd["smart_experience"].click_attention_focus_toggle_button_toggle_button_state()
                logging.info(f"Clicked to turn on after toggling off. Attempt {i+1}/5.")
        serial_number = self.fc.get_windows_serial_number()
        custom_filter = {    
        "version": "2.0.0",   
        "viewHierarchy": ["base:/", "mfe:/SmartExperiences/FeatureCard/"],
        "viewName": "FeatureCard", 
        "action": "OnChange",
        "actionDetail": "On",
        "viewModule": "SmartExperiences",
        "controlName": "AttentionfocusToggle",
        "controlLabel": "AttentionfocusToggle",
        "serial_number": serial_number
        }
        analytics_test.custom_filter_json_file("resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", custom_filter)
        analytics_test.execute_analytics_test_and_validate_result(query_start_time, "resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", "vision_ai", 5)

    @pytest.mark.analytics
    def test_04_attention_focus_off_toggle_switch_event_C52212719(self):
        self.fc.fd["smart_experience"].verify_attention_focus_text_ltwo_page(), "Attention Focus card text is not displayed"
        query_start_time = datetime.now(timezone.utc).isoformat()
        for i in range(5):
            current_state = self.fc.fd["smart_experience"].get_attention_focus_toggle_button_state()
            if current_state == "1":
                self.fc.fd["smart_experience"].click_attention_focus_toggle_button_toggle_button_state()
                logging.info(f"Clicked to turn on. Attempt {i+1}/5.")
            else:
                self.fc.fd["smart_experience"].click_attention_focus_toggle_button_toggle_button_state()
                self.fc.fd["smart_experience"].click_attention_focus_toggle_button_toggle_button_state()
                logging.info(f"Clicked to turn on after toggling off. Attempt {i+1}/5.")
        serial_number = self.fc.get_windows_serial_number()
        custom_filter = {    
        "version": "2.0.0",   
        "viewHierarchy": ["base:/", "mfe:/SmartExperiences/FeatureCard/"],
        "viewName": "FeatureCard", 
        "action": "OnChange",
        "actionDetail": "Off",
        "viewModule": "SmartExperiences",
        "controlName": "AttentionfocusToggle",
        "controlLabel": "AttentionfocusToggle",
        "serial_number": serial_number
        }
        analytics_test.custom_filter_json_file("resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", custom_filter)
        analytics_test.execute_analytics_test_and_validate_result(query_start_time, "resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", "vision_ai", 5)

    @pytest.mark.analytics
    def test_05_intelligent_dynamic_contrast_on_toggle_switch_event_C52212720(self):
        self.fc.fd["smart_experience"].verify_intelligent_dynamic_contrast_text_ltwo_page(), "Intelligent Dynamic Contrast card text is not displayed"
        query_start_time = datetime.now(timezone.utc).isoformat()
        for i in range(5):
            current_state = self.fc.fd["smart_experience"].get_intelligent_dynamic_contrast_toggle_button_state()
            if current_state == "0":
                self.fc.fd["smart_experience"].click_intelligent_dynamic_contrast_toggle_button()
                logging.info(f"Clicked to turn on. Attempt {i+1}/5.")
            else:
                self.fc.fd["smart_experience"].click_intelligent_dynamic_contrast_toggle_button()
                self.fc.fd["smart_experience"].click_intelligent_dynamic_contrast_toggle_button()
                logging.info(f"Clicked to turn on after toggling off. Attempt {i+1}/5.")
        serial_number = self.fc.get_windows_serial_number()
        custom_filter = {    
        "version": "2.0.0",   
        "viewHierarchy": ["base:/", "mfe:/SmartExperiences/FeatureCard/"],
        "viewName": "FeatureCard", 
        "action": "OnChange",
        "actionDetail": "On",
        "viewModule": "SmartExperiences",
        "controlName": "IntelligentdynamiccontrastToggle",
        "controlLabel": "IntelligentdynamiccontrastToggle",
        "serial_number": serial_number
        }
        analytics_test.custom_filter_json_file("resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", custom_filter)
        analytics_test.execute_analytics_test_and_validate_result(query_start_time, "resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", "vision_ai", 5)

    @pytest.mark.analytics
    def test_06_intelligent_dynamic_contrast_off_toggle_switch_event_C52212721(self):
        self.fc.fd["smart_experience"].verify_intelligent_dynamic_contrast_text_ltwo_page(), "Intelligent Dynamic Contrast card text is not displayed"
        query_start_time = datetime.now(timezone.utc).isoformat()
        for i in range(5):
            current_state = self.fc.fd["smart_experience"].get_intelligent_dynamic_contrast_toggle_button_state()
            if current_state == "1":
                self.fc.fd["smart_experience"].click_intelligent_dynamic_contrast_toggle_button()
                logging.info(f"Clicked to turn on. Attempt {i+1}/5.")
            else:
                self.fc.fd["smart_experience"].click_intelligent_dynamic_contrast_toggle_button()
                self.fc.fd["smart_experience"].click_intelligent_dynamic_contrast_toggle_button()
                logging.info(f"Clicked to turn on after toggling off. Attempt {i+1}/5.")
        serial_number = self.fc.get_windows_serial_number()
        custom_filter = {    
        "version": "2.0.0",   
        "viewHierarchy": ["base:/", "mfe:/SmartExperiences/FeatureCard/"],
        "viewName": "FeatureCard", 
        "action": "OnChange",
        "actionDetail": "Off",
        "viewModule": "SmartExperiences",
        "controlName": "IntelligentdynamiccontrastToggle",
        "controlLabel": "IntelligentdynamiccontrastToggle",
        "serial_number": serial_number
        }
        analytics_test.custom_filter_json_file("resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", custom_filter)
        analytics_test.execute_analytics_test_and_validate_result(query_start_time, "resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", "vision_ai", 5)

    @pytest.mark.analytics
    def test_07_onlooker_on_toggle_switch_event_C52212722(self):
        self.fc.fd["smart_experience"].verify_onlooker_detection_text_ltwo_page(), "Onlooker card text is not displayed"
        query_start_time = datetime.now(timezone.utc).isoformat()
        for i in range(5):
            current_state = self.fc.fd["smart_experience"].get_onlooker_detection_toggle_button_state()
            if current_state == "0":
                self.fc.fd["smart_experience"].click_onlooker_detection_toggle_button()
                logging.info(f"Clicked to turn on. Attempt {i+1}/5.")
            else:
                self.fc.fd["smart_experience"].click_onlooker_detection_toggle_button()
                self.fc.fd["smart_experience"].click_onlooker_detection_toggle_button()
                logging.info(f"Clicked to turn on after toggling off. Attempt {i+1}/5.")
        serial_number = self.fc.get_windows_serial_number()
        custom_filter = {    
        "version": "2.0.0",   
        "viewHierarchy": ["base:/", "mfe:/SmartExperiences/FeatureCard/"],
        "viewName": "FeatureCard", 
        "action": "OnChange",
        "actionDetail": "On",
        "viewModule": "SmartExperiences",
        "controlName": "OnlookerdetectionToggle",
        "controlLabel": "OnlookerdetectionToggle",
        "serial_number": serial_number
        }
        analytics_test.custom_filter_json_file("resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", custom_filter)
        analytics_test.execute_analytics_test_and_validate_result(query_start_time, "resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", "vision_ai", 5)

    @pytest.mark.analytics
    def test_08_onlooker_off_toggle_switch_event_C52212723(self):
        self.fc.fd["smart_experience"].verify_onlooker_detection_text_ltwo_page(), "Onlooker card text is not displayed"
        query_start_time = datetime.now(timezone.utc).isoformat()
        for i in range(5):
            current_state = self.fc.fd["smart_experience"].get_onlooker_detection_toggle_button_state()
            if current_state == "1":
                self.fc.fd["smart_experience"].click_onlooker_detection_toggle_button()
                logging.info(f"Clicked to turn on. Attempt {i+1}/5.")
            else:
                self.fc.fd["smart_experience"].click_onlooker_detection_toggle_button()
                self.fc.fd["smart_experience"].click_onlooker_detection_toggle_button()
                logging.info(f"Clicked to turn on after toggling off. Attempt {i+1}/5.")
        serial_number = self.fc.get_windows_serial_number()
        custom_filter = {    
        "version": "2.0.0",   
        "viewHierarchy": ["base:/", "mfe:/SmartExperiences/FeatureCard/"],
        "viewName": "FeatureCard", 
        "action": "OnChange",
        "actionDetail": "Off",
        "viewModule": "SmartExperiences",
        "controlName": "OnlookerdetectionToggle",
        "controlLabel": "OnlookerdetectionToggle",
        "serial_number": serial_number
        }
        analytics_test.custom_filter_json_file("resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", custom_filter)
        analytics_test.execute_analytics_test_and_validate_result(query_start_time, "resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", "vision_ai", 5)

    @pytest.mark.analytics
    def test_09_enable_screen_blur_on_toggle_switch_event_C52212726(self):
        self.fc.fd["smart_experience"].verify_enable_screen_blur_ltwo_page(), "Enable Screen Blur card text is not displayed"
        current_state = self.fc.fd["smart_experience"].get_onlooker_detection_toggle_button_state()
        if current_state == "0":
                self.fc.fd["smart_experience"].click_onlooker_detection_toggle_button()
        query_start_time = datetime.now(timezone.utc).isoformat()
        for i in range(5):
            current_state = self.fc.fd["smart_experience"].get_enable_screen_blur_toggle_button_state()
            if current_state == "0":
                self.fc.fd["smart_experience"].click_enable_screen_blur_toggle_button()
                logging.info(f"Clicked to turn on. Attempt {i+1}/5.")
            else:
                self.fc.fd["smart_experience"].click_enable_screen_blur_toggle_button()
                self.fc.fd["smart_experience"].click_enable_screen_blur_toggle_button()
                logging.info(f"Clicked to turn on after toggling off. Attempt {i+1}/5.")
        serial_number = self.fc.get_windows_serial_number()
        custom_filter = {    
        "version": "2.0.0",   
        "viewHierarchy": ["base:/", "mfe:/SmartExperiences/FeatureCard/"],
        "viewName": "FeatureCard", 
        "action": "OnChange",
        "actionDetail": "On",
        "viewModule": "SmartExperiences",
        "controlName": "ScreenblurToggle",
        "controlLabel": "ScreenblurToggle",
        "serial_number": serial_number
        }
        analytics_test.custom_filter_json_file("resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", custom_filter)
        analytics_test.execute_analytics_test_and_validate_result(query_start_time, "resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", "vision_ai", 5)

    @pytest.mark.analytics
    def test_10_enable_screen_blur_off_toggle_switch_event_C52212727(self):
        self.fc.fd["smart_experience"].verify_enable_screen_blur_ltwo_page(), "Enable Screen Blur card text is not displayed"
        current_state = self.fc.fd["smart_experience"].get_onlooker_detection_toggle_button_state()
        if current_state == "0":
                self.fc.fd["smart_experience"].click_onlooker_detection_toggle_button()
        query_start_time = datetime.now(timezone.utc).isoformat()
        for i in range(5):
            current_state = self.fc.fd["smart_experience"].get_enable_screen_blur_toggle_button_state()            
            if current_state == "1":
                self.fc.fd["smart_experience"].click_enable_screen_blur_toggle_button()
                logging.info(f"Clicked to turn on. Attempt {i+1}/5.")
            else:
                self.fc.fd["smart_experience"].click_enable_screen_blur_toggle_button()
                self.fc.fd["smart_experience"].click_enable_screen_blur_toggle_button()
                logging.info(f"Clicked to turn on after toggling off. Attempt {i+1}/5.")
        serial_number = self.fc.get_windows_serial_number()
        custom_filter = {    
        "version": "2.0.0",   
        "viewHierarchy": ["base:/", "mfe:/SmartExperiences/FeatureCard/"],
        "viewName": "FeatureCard", 
        "action": "OnChange",
        "actionDetail": "Off",
        "viewModule": "SmartExperiences",
        "controlName": "ScreenblurToggle",
        "controlLabel": "ScreenblurToggle",
        "serial_number": serial_number
        }
        analytics_test.custom_filter_json_file("resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", custom_filter)
        analytics_test.execute_analytics_test_and_validate_result(query_start_time, "resources/test_data/hpx_rebranding/analytics_opensearch_filter/smart_experience_filter.json", "vision_ai", 5)