...
pytest.app_info = "HPX"
class Test_Suite_10_Sanity_Scan_E2E_Flow_1(object):
    ...
    @pytest.mark.smoke
    def test_01_happy_path_scan_edit_save_pdf_C43738391(self):
        """
        [Happy path] Scan, edit and then save to PDF.

        TestRails -> https://hp-testrail.external.hp.com/index.php?/cases/view/43738391
        """
    ...
    @pytest.mark.smoke
    def test_02_happy_path_scan_edit_save_image_C43738392(self):
        """
        [Happy path] Scan, edit and then save to images. 

        TestRails -> https://hp-testrail.external.hp.com/index.php?/cases/view/43738392
        """
        self.fc.restart_hpx()
        self.go_through_flow_to_scanner_screen()
        self.__randomize_scanner_source()
        ...
        self.__randomize_scan_pages()
        self.__randomize_edit_image()
        self.__scan_save_flow(file_type=self.fc.fd["scan"].IMAGE_JPG)
    ...
    def __review_saved_file(self, multi_files):
        ...
        try:
            self.driver.ssh.send_command("Remove-Item -Path '{}' -Force -Recurse -ErrorAction Stop".format(file_path), timeout=60, raise_e=False)
        except Exception as e:
            logging.warning(f"First attempt to delete file failed: {e}. Retrying...")
            sleep(2)
            try:
                self.driver.ssh.send_command("Remove-Item -Path '{}' -Force -Recurse -ErrorAction SilentlyContinue".format(file_path), timeout=60, raise_e=False)
            except Exception as e2:
                logging.error(f"Failed to delete file after retry: {e2}. File may remain: {file_path}")
