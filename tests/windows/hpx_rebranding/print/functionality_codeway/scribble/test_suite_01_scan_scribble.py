...
class Test_Suite_03_Scan_Scribble(object):
    ...
    def test_01_verify_multiple_scan_thumbnail_view_C43738610(self):
        self.devicesMFE.click_windows_dummy_printer(self.printer_name)
        assert self.devicesDetailsMFE.verify_scan_tile(), "Scan tile is not displayed on device details page."
        ...
        for idx in (1, 2):
            self.scan.select_thumbnail_item(idx)
        assert self.scan.verify_thumbnail_scribble_btn_status(enabled=False), "Thumbnail Scribble status button is enabled"