...
pytest.app_info = "HPX"
class Test_Suite_03_Scan_Share_File_Name(object):
    ...
    @pytest.mark.parametrize("file, file_path", [("photo", "photo_path"), ("doc", "doc_path"), ("receipt", "receipt_path"), ("business", "business_path"), ("hand", "hand_path")])
    @pytest.mark.regression
    def test_02_scan_a_file_to_check_file_name_text_c43738690(self, file, file_path):
        """
        Check File Name Text box from Share flyout, verify correct file name shows

        Scan a Photo image and then click "Share" button on "Preview" screen to check File Name Text box from Share flyout.
        Scan a Document file and check File Name from Share flyout.
        Scan a Receipt file and check File Name from Share flyout.
        Scan a BusinessCard and check File Name from Share flyout.
        Scan a Handwritten file and check File Name from Share flyout.
        ...
        TestRails -> https://hp-testrail.external.hp.com/index.php?/cases/view/43738690 (a file)
        """
    ...
    @pytest.mark.parametrize("file, file_path", [("photo", "photo_path"), ("doc", "doc_path"), ("receipt", "receipt_path"), ("business", "business_path"), ("hand", "hand_path")])
    @pytest.mark.regression
    def test_03_scan_mixed_files_to_check_file_name_text_c43738690(self, file, file_path):
        """
        Check File Name Text box from Save flyout, verify correct file name shows

        Scan mixed files, and the first file is Photo image, check File Name from Share flyout.
        Scan mixed files, and the first file is Document file, check File Name from Share flyout.
        Scan mixed files, and the first file is Receipt file, check File Name from Share flyout.
        Scan mixed files, and the first file is BusinessCard, check File Name from Share flyout.
        Scan mixed files, and the first file is handwritten file, check File Name from Share flyout.
        ...
        TestRails -> https://hp-testrail.external.hp.com/index.php?/cases/view/43738690 (mixed files)
        """
        self.fc.check_files_exist(self.files[file], self.files_path[file_path])
    ...
    def __scan_file_and_check_file_name_text(self, file, add=True):
        ...
        pattern = r'\d{2}-([0-1][1-9]|1[0-2])-([0-3][0-9])_\d{6}'
        if file == w_const.TEST_DATA.AUTUMN_JPG:
            ...
            start_s = 'Handwritten'
        try:
            logging.info("File name is: {}".format(file_name))
            assert re.match('^{}_20{}$'.format(start_s, pattern), file_name) is not None
        finally:
