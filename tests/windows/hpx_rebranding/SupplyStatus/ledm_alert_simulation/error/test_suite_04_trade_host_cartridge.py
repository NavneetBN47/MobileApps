...
class Test_Suite_04_Trade_Host_Cartridge(object):
    ...
    def test_01_trade_cartridge_when_expect_host_65590(self):
        supply_error_title, supply_error_img, error_detailed_info = self.fc.read_supplies_status_json_files(ioref=65590, type="error")
        success = ErrorSimulatorLEDM.trade_cartridge_when_expect_host_65590(self.error_manager, color="k")
        assert success, "Failed to set trade_cartridge_when_expect_host_65590 error"
    ...
    def test_02_host_cartridge_when_expect_trade_65591(self):
        supply_error_title, supply_error_img, error_detailed_info = self.fc.read_supplies_status_json_files(ioref=65591, type="error")
        success = ErrorSimulatorLEDM.host_cartridge_when_expect_trade_65591(self.error_manager, color="k")
        assert success, "Failed to set host_cartridge_when_expect_trade_65591 error"
        assert self.supplies_status.verify_printer_card_present()
        assert self.fc.validate_supplies_status_headers_home_page(supply_error_title, supply_error_img, type="error")
        self.supplies_status.click_printer_card()
        assert self.supplies_status.verify_error_icon_in_pdp()
    ...
    @pytest.mark.verona
    @pytest.mark.palermo_fast
    def test_03_hp_altered_supply_65590_65592(self):
        supply_error_title, supply_error_img, error_detailed_info = self.fc.read_supplies_status_json_files(ioref=65590, type="error")
        success = ErrorSimulatorLEDM.hp_altered_supply_65590_65592(self.error_manager, color="k")
        assert success, "Failed to set hp_altered_supply_65590_65592 error"
        assert self.supplies_status.verify_printer_card_present()
        assert self.fc.validate_supplies_status_headers_home_page(supply_error_title, supply_error_img, type="error")
        self.supplies_status.click_printer_card()
        assert self.supplies_status.verify_error_icon_in_pdp()
