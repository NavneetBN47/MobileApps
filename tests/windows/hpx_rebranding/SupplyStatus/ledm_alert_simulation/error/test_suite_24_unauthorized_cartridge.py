...
class Test_Suite_24_Unauthorized_Cartridge(object):
    ...
    def test_01_unauthorized_supply_65926(self):
        supply_error_title, supply_error_img, error_detailed_info = self.fc.read_supplies_status_json_files(ioref=65926, type="error")
        success = ErrorSimulatorLEDM.unauthorized_supply_65926(self.error_manager, color="k")
        assert success, "Failed to set unauthorized_supply_65926 error"
        ...
        assert self.supplies_status.verify_error_alert_title_in_pdp(supply_error_title), "Error alert title in PDP is incorrect"
