...
class Test_Suite_07_Genuine_supplyflow(object):
    ...
    @pytest.mark.selene
    def test_01_genuinehp_supplyflow_66101(self):
        info_title, info_img, info_detailed_body = self.fc.read_supplies_status_json_files(ioref=66101, type="info")
        success = InformationSimulatorCDM.genuinehp_supplyflow_66101(self.error_manager, color="k")
    ...
    @pytest.mark.jupiter
    def test_02_ids_delayed_backto_hp_66516(self):
        info_title, info_img, info_detailed_body = self.fc.read_supplies_status_json_files(ioref=66516, type="info")
        success = InformationSimulatorCDM.ids_delayed_backto_hp_66516(self.error_manager, color="k")
        assert success, "Failed to set ids delayed backto hp info"
        assert self.supplies_status.verify_printer_card_present()
        assert self.fc.validate_supplies_status_headers_home_page(info_title, info_img, type="info")
        self.supplies_status.click_printer_card()
        assert self.supplies_status.verify_info_icon_in_pdp()
