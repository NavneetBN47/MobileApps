...
class Test_Suite_06_Print_Head(object):
    ...
    def test_03_ph_replacement_flow_66640(self):
        info_title, info_img, info_detailed_body = self.fc.read_supplies_status_json_files(ioref=66640, type="info")
        success = InformationSimulatorCDM.ph_replacement_flow_66640(self.error_manager,color="k")
        assert success, "Failed to set ph replacement flow info"
        assert self.supplies_status.verify_printer_card_present()
        assert self.fc.validate_supplies_status_headers_home_page(info_title, info_img, type="info")
        self.supplies_status.click_printer_card()
        assert self.supplies_status.verify_info_icon_in_pdp()
