...
class Test_Suite_10_Out_Of_Ink(object):
    ...
    @pytest.mark.pixiu
    def test_01_outofink_66019(self):
        error_title, error_img, error_detailed_info = self.fc.read_supplies_status_json_files(ioref=66019, type="error")
        success = ErrorSimulatorCDM.outofink_66019(self.error_manager, color="k")
        assert success, "Failed to set out of ink error"
    ...
    @pytest.mark.jupiter
    def test_02_outofink_66537(self):
        error_title, error_img, error_detailed_info = self.fc.read_supplies_status_json_files(ioref=66537, type="error")
        success = ErrorSimulatorCDM.outofink_66537(self.error_manager, color="k")
        assert success, "Failed to set out of ink error"
        assert self.supplies_status.verify_printer_card_present()
        assert self.fc.validate_supplies_status_headers_home_page(error_title, error_img, type="error")
        self.supplies_status.click_printer_card()
    ...
    @pytest.mark.pixiu
    def test_03_idsstartup_blocked_loi_flow_66440(self):
        error_title, error_img, error_detailed_info = self.fc.read_supplies_status_json_files(ioref=66440, type="error")
        success = ErrorSimulatorCDM.idsstartup_blocked_loi_flow_66440(self.error_manager, color="k")
        assert success, "Failed to set IdsStartup Blocked Loi Flow error"
        assert self.supplies_status.verify_printer_card_present()
        assert self.fc.validate_supplies_status_headers_home_page(error_title, error_img, type="error")
        self.supplies_status.click_printer_card()
        assert self.supplies_status.verify_error_icon_in_pdp()
    ...
    @pytest.mark.pixiu
    def test_04_outofink_66618(self):
        error_title, error_img, error_detailed_info = self.fc.read_supplies_status_json_files(ioref=66618, type="error")
        success = ErrorSimulatorCDM.outofink_66618(self.error_manager, color="k")
        assert success, "Failed to set out of ink error"
        assert self.supplies_status.verify_printer_card_present()
        assert self.fc.validate_supplies_status_headers_home_page(error_title, error_img, type="error")
        self.supplies_status.click_printer_card()
