...
class Test_Suite_05_Tank_Very_Low(object):
    ...
    @pytest.mark.pixiu
    def test_01_cartridge_very_low_66018(self):
        warning_title, warning_img, warning_detailed_body = self.fc.read_supplies_status_json_files(ioref=66018, type="warning")
        success = WarningSimulatorCDM.cartridge_very_low_66018(self.error_manager, color="k")
    ...
    @pytest.mark.lebi_wireless_flex
    @pytest.mark.pixiu
    def test_02_tank_very_low_66018(self):
        warning_title, warning_img, warning_detailed_body = self.fc.read_supplies_status_json_files(ioref=66018, type="warning")
        success = WarningSimulatorCDM.tank_very_low_66018(self.error_manager, color="k")
        assert success, "Failed to set tank very low error"
        assert self.supplies_status.verify_printer_card_present()
        assert self.fc.validate_supplies_status_headers_home_page(warning_title, warning_img, type="warning")
        self.supplies_status.click_printer_card()
        self.supplies_status.verify_warning_icon_in_pdp()
