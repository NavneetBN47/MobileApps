...
class Test_Suite_03_Used_Or_Refilled_Cartridge(object):
    ...
    def test_01_validate_hp_altered_supply_refilled_flow_no_66443(self):
        warning_title, warning_img, warning_detailed_body = self.fc.read_supplies_status_json_files(ioref=66443, type="warning")
    ...
    @pytest.mark.pixiu
    def test_02_calibration_required_66495(self):
        warning_title, warning_img, warning_detailed_body = self.fc.read_supplies_status_json_files(ioref=66495, type="warning")
        success = WarningSimulatorCDM.calibration_required_66495(self.error_manager,color='k')
        ...
        self.supplies_status.click_warning_alert_in_pdp()
        assert self.supplies_status.verify_cartridge_colour_icon('k')
        assert self.supplies_status.verify_alert_title_after_clicked() == warning_title, "Alert title Mismatched after clicked"   
        assert self.fc.verify_detailed_body_about_printer_status_alert(warning_detailed_body), f"Detailed {warning_title} is incorrect"
