...
class Test_Suite_01_Manage_Privacy_Settings(object):
    ...
    def test_01_verify_manage_privacy_settings_page_ui_C58769277(self):
        ...
        logging.info("Manage Privacydescription text: {}".format(self.hpx_settings.get_manage_privacy_description_text()))

    @pytest.mark.regression
    def test_02_verify_hp_privacy_statement_link_C59486532(self):
        assert self.devicesMFE.verify_profile_icon_show_up(), "Profile icon missing"
        self.devicesMFE.click_profile_button()
        assert self.profile.verify_profile_setting_page_content(), "Profile Settings page content missing"
        self.profile.click_profile_settings_btn()
