...
class Test_poobe_Flow(object):
    ...
    def class_setup(self, poobe_test_setup):
        ...
        self.country, self.language = "canada", "en"
        self.printer_type = "personal"
        ...
        self.printer_info = self.fc.generate_simulator_printer(self.printer_profile, self.stack, self.biz_model, country=self.country, language=self.language) if self.printer_profile in self.fc.dual_sku_printers else None
    ...
    def test_01_hp_smart_account_dashboard_personal(self): 
        if self.printer_info is None: pytest.skip()
        self.fc.clear_browsing_data_and_relaunch_flow(self.browser, url=self.p_oobe.poobe_url.replace("us/en", "ca/en"))
        ...
        if "ca/en" not in self.driver.current_url:
            self.driver.navigate(self.driver.current_url.replace("us/en", "ca/en"))
