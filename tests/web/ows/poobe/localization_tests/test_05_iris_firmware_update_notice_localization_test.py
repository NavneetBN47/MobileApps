...
class Test_01_Portal_OOBE_Localization(object):
    ...
    def class_setup(self, poobe_test_setup, request):
        ...
        self.sim_printer_info = ows_utility.create_simulated_gen2_printer(stack=self.stack, profile=self.printer_profile, biz_model=self.biz_model, offer=0, country=False, language=False)
        
        """
        IRIS Firmware Update notice only for Flex /connect Flow
        """
    
    def test_01_iris_firmware_update_notice(self):
        if self.biz_model == "E2E":
            pytest.skip()
        file_name = ma_misc.web_localization_path_builder(self.driver, self.file_key["portal_oobe"]["iris_firmware_update"]["Flex_connect"])
        ...
        self.fc.fd["iris_fw_update_notice"].verify_fw_update_modal_page()
        res =[]
        res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "page_header", raise_e=False))
        res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "page_content", raise_e=False))
        res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "features[0]_text", raise_e=False))
        if self.locale not in self.hp_plus_countries: # IRIS page shows more features for Non HP+ Regions 
            res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "features[2]_text", raise_e=False))
            res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "features[3]_text", raise_e=False))
            res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "features[4]_text", raise_e=False))
            res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "features[5]_text", raise_e=False))
        res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "page_confirm_and_continue_button", raise_e=False))
        res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "page_cancel_and_setup_manually_button", raise_e=False))
        self.fc.fd["iris_fw_update_notice"].click_cancel_and_manual_setup()
        self.fc.fd["iris_fw_update_notice"].verify_cancel_and_setup_manually_modal()
        res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "modal_header", raise_e=False))
        res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "modal_content_before_features", raise_e=False))
        res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "modal_content_after_features", raise_e=False))
        res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "modal_continue_online_setup_button", raise_e=False))
        res.append(self.fc.fd["iris_fw_update_notice"].string_validation(spec_data, "modal_cancel_online_setup_button", raise_e=False))
        self.fc.fd["iris_fw_update_notice"].click_continue_online_setup_overlay_model_btn()
