...
stack = str(tree.xpath('//h1/text()')).split(",")[0].split("=")[-1].split("_")[1]

try:
    platform = tree.xpath('//h1/text()')[0].split(',')[2].split('=')[1].split('.')[0].split('&&')[-1].split("_")[0]
...
results_dir = "/work/exec/workspace/SHI_WINDOWS_HPX_REBRAND_MAT/mat_results/"
if not os.path.exists(results_dir):
    os.makedirs(results_dir)

try:
    with open("/work/exec/workspace/SHI_WINDOWS_HPX_REBRAND_MAT/mat_results/" + platform + '_results.txt', 'r') as file:
        result_dict = json.load(file)
except FileNotFoundError:
    result_dict = {"pie": "Passed", "stage": "Passed", "production": "Passed"}

try:
    with open("/work/exec/workspace/SHI_WINDOWS_HPX_REBRAND_MAT/mat_results/" + 'stack_version.txt', 'r') as file:
...
with open("/work/exec/workspace/SHI_WINDOWS_HPX_REBRAND_MAT/mat_results/" + platform + '_results.txt', 'w') as file:
    json.dump(result_dict, file)

with open("/work/exec/workspace/SHI_WINDOWS_HPX_REBRAND_MAT/mat_results/" + 'stack_version.txt', 'w') as file:
...
folder_path = "/work/exec/workspace/SHI_WINDOWS_HPX_REBRAND_MAT/mat_results/"
...
env = Environment(loader=FileSystemLoader('/work/exec/shanghai/MobileApps/resources/test_data/hpx_rebranding/report_template/'))
template = env.get_template('report_template.html')
report = template.render(data=display_data_results,  additional_data=version_tuples)



with open("/work/exec/workspace/SHI_WINDOWS_HPX_REBRAND_MAT/mat_results/mat_results.html", 'w') as f:
    f.write(report)
    f.close()