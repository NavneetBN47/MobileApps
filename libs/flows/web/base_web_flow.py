...
class BaseWebFlow(BaseFlow):
    ...
    def __init__(self, driver, context=None, url=None, window_name="main"):
        ...
        super(BaseWebFlow, self).__init__(driver)
        self.logging_ignore_methods.append("wrapper")
        self.wn = window_name
        self.load_web_shared_ui()
        if context is not None:
            self.context = context
        self.url = url

    #load jshell shared objects
    def load_web_shared_ui(self):
        ui_map = self.load_ui_map(system="WEB", project="jshell", flow_name="shared_obj")
        self.driver.load_ui_map("smb", "shared_obj", ui_map)
        return True
        
    ######################## Common object verify ############################################
    def verify_table_loaded(self):
        #Only works with table with a link in it, which is most tables
        #30 second timeout is the maxium timeout do not change
        return self.driver.wait_for_object("_shared_table_entries_with_link", timeout=30)
